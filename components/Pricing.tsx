        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div>
            <h2 className="text-5xl md:text-6xl font-black italic uppercase leading-tight text-white mb-2">
              투명하고<br />
              <span className="text-sky-400">합리적인</span><br />
              가격
            </h2>
            ...
            <div className="border border-white/5 bg-white/[0.02] p-8 mb-6">
              <div className="flex items-end gap-2 mb-1">
                <span className="text-[clamp(60px,10vw,100px)] font-black italic text-sky-400 leading-none">
                  ₩20K
                </span>
              </div>
              ...
              <div className="space-y-3 mb-8">
                {included.map((item, i) => (
                  <div key={i} className="flex items-center gap-4">
                    <div className="w-4 h-[2px] bg-sky-400/40 flex-shrink-0" />
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="border border-white/5 px-5 py-4">
                <div className="text-2xl font-black italic text-sky-400">1H</div>
                <div className="text-[10px] uppercase tracking-widest text-gray-600 mt-1">집중 트레이닝</div>
              </div>
              <div className="border border-white/5 px-5 py-4">
                <div className="text-2xl font-black italic text-sky-400">AM8</div>
              ...
              {compare.map((item, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    {item.highlight && (
                      <div className="w-1.5 h-6 bg-sky-400 flex-shrink-0" />
                    )}
                    <div>
                      ...
                      {item.highlight && (
                        <div className="text-[10px] uppercase tracking-widest text-sky-400/60 font-bold">
                          {item.note}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="text-right">
                    <span className={`font-black italic text-xl ${
                      item.highlight ? "text-sky-400" : "text-gray-600"
                    }`}>
                      {item.price}
                    </span>
                    <span className={`text-xs ${item.highlight ? "text-sky-400/60" : "text-gray-700"}`}>
                      {item.per}
                    </span>
                  </div>
                </div>
