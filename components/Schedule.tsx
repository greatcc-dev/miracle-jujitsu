              ...
              {timeline.map((item, i) => (
                <div
                  ...
                  className={`flex gap-6 py-5 border-t border-white/5 group ${
                    item.active ? "border-yellow-400/20" : ""
                  }`}
                >
                  {/* Time */}
                  <div className={`font-mono font-black text-sm w-14 flex-shrink-0 pt-0.5 ${
                    item.active ? "text-yellow-400" : "text-gray-600"
                  }`}>
                    {item.time}
                  </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {targets.map((item, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 border border-white/5 hover:border-yellow-400/20 px-4 py-4 transition-colors duration-200 group"
                >
            ...
            <div className="mt-8 border border-yellow-400/20 bg-yellow-400/[0.03] p-6">
              <div className="flex items-start gap-4">
                <span className="text-yellow-400 text-2xl font-black italic leading-none">!</span>
                <div>
